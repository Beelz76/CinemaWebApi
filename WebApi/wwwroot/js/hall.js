async function getHalls() {
	try {
		const response = await fetch(
			"https://localhost:7172/api/Hall/GetHalls",
			{
				method: "GET",
				headers: {
...
async function createHall() {
	...
	try {
		const response = await fetch(
			`https://localhost:7172/api/Hall/CreateHall?name=${hallName}`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
async function updateHall() {
	...
	const selectedCheckboxes = document.querySelectorAll(
		'#hallTable input[type="checkbox"]:checked'
	);
	...
	try {
		const response = await fetch(
			`https://localhost:7172/api/Hall/UpdateHall?hallUid=${uid}&name=${hallName}`,
			{
				method: "PUT",
				headers: {
					"Content-Type": "application/json",

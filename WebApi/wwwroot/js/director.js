...
async function createDirector() {
    ...
    try {
        const response = await fetch(`https://localhost:7172/api/Director/Createdirector?fullName=${directorFullName}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('userToken')}`
...
async function updateDirector() {
    ...
    try {
        const response = await fetch(`https://localhost:7172/api/Director/UpdateDirector?directorUid=${uid}&fullName=${directorFullName}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('userToken')}`,
